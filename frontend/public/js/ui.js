import { state, NAV_LINKS, isInternalRole, isSupplierRole } from './state.js';
  ...
  container.innerHTML = `
    ...
    <div class="nav-actions">
      <div class="notification-center" id="notificationCenter">
        <button type="button" class="notification-toggle" id="notificationToggle" aria-label="Benachrichtigungen">
          <span class="notification-bell" aria-hidden="true">
            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="#000000" height="44px" width="44px" version="1.1" id="Capa_1" viewBox="0 0 611.999 611.999" xml:space="preserve" style="
    width: 20px;
">
              <g>
                <g>
        ...
        <div class="notification-panel" id="notificationPanel" aria-hidden="true">
          <header class="notification-panel-head">
            ...
            <a class="ghost small" href="/benachrichtigungen.html">Alle Benachrichtigungen</a>
          </header>
          <ul class="notification-list" id="notificationList"></ul>
          <p class="notification-empty" id="notificationEmptyState">Keine Benachrichtigungen</p>
        </div>
      </div>
      <span class="user-label" id="userLabel">
        <span class="user-label-prefix" data-i18n-key="Angemeldet als:">Angemeldet als:</span>
        <strong id="userLabelName">-</strong>
      </span>
...
export function renderSidebar(activePage) {
  ...
  const notificationClasses = ['sidebar-link-notifications'];
  if (activePage === 'notifications') notificationClasses.push('active');
  container.innerHTML = `
    <nav>${links}</nav>
    <div class="sidebar-footer">
      <a href="/benachrichtigungen.html" class="${notificationClasses.join(' ')}">Benachrichtigungen</a>
    </div>
  `;
}
...
export function applyRoleVisibility() {
  const isBate = isInternalRole(state.user?.role);
  ...
  document.querySelectorAll('.supplier-hidden').forEach((element) => {
    const preferredDisplay = element.dataset.display || 'block';
    element.style.display = isSupplier ? 'none' : preferredDisplay;
  });
}
